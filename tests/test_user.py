...
class TestUser(unittest.TestCase):
    ...
    def test_to_dict_method(self):
        ...
        expected_keys = ['id', 'created_at', 'updated_at', 'first_name', 'last_name', 'email', 'password']

        for key in expected_keys:
            self.assertIn(key, user_dict)

        date_format = "%Y-%m-%dT%H:%M:%S.%f"
        self.assertEqual(user.created_at.strftime(date_format), user_dict['created_at'])
        self.assertEqual(user.updated_at.strftime(date_format), user_dict['updated_at'])
